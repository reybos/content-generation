...
import { FileService, ImageService, VideoService, LockService, StateService, UniversalWorker, VideoWorker } from "./services";
 ...
 * Delegates image generation to UniversalWorker and video generation to VideoWorker
 */
        ...
        while (this.isRunning) {
            try {
                ...
                if (unprocessedFiles.length > 0) {
                    this.logger.info(`Found ${unprocessedFiles.length} unprocessed JSON files, running universal worker for image generation`);
                    await universalWorker.start();
                    continue;
                }

                // 2. Video Generation Phase: обрабатываем папки с картинками и генерируем видео
                const unprocessedFolders = await this.fileService.getUnprocessedFolders();
                if (unprocessedFolders.length > 0) {
                    this.logger.info(`Found ${unprocessedFolders.length} unprocessed folders, running video worker for video generation`);
